...
export class AuthService implements IAuthService {
  ...
  async googleAuth({
    ...
    if (!email) {
      ...
      throw new AppError(
        MESSAGES.AUTH.GOOGLE_EMAIL_REQUIRED,
    ...
    if (existingUser && existingUser.role !== role) {
      ...
      throw new AppError(MESSAGES.AUTH.ROLE_CONFLICT, HttpStatus.CONFLICT);
    }
  ...
  async verifyOtp(dto: verifyOtpDto): Promise<verifyOtpResult> {
    ...
    if (!storedOtp) {
      ...
      throw new AppError(MESSAGES.AUTH.OTP_EXPIRED, HttpStatus.BAD_REQUEST);
    }
    if (storedOtp !== dto.otp) {
      ...
      throw new AppError(MESSAGES.AUTH.INVALID_OTP, HttpStatus.BAD_REQUEST);
    }
  ...
  async resendOtp(dto: resendOtpDto): Promise<{ success: boolean }> {
    ...
    if (!existing) {
      ...
      throw new AppError(
        MESSAGES.AUTH.LANDLORD_NOT_FOUND,
  ...
  async login(dto: LoginDto): Promise<LoginResultDto> {
    ...
    if (!user) {
      ...
      throw new AppError(
        MESSAGES.AUTH.LOGIN_EMAIL_NOT_FOUND,
        HttpStatus.UNAUTHORIZED,
      );
    }
    ...
    if (!isValidPassword) {
      ...
      throw new AppError(MESSAGES.AUTH.WRONG_PASSWORD, HttpStatus.UNAUTHORIZED);
    }
  ...
  async resetPassword(dto: resetPasswordDto): Promise<{ success: boolean }> {
    ...
    if (isSameAsOldPassword) {
      throw new AppError(MESSAGES.AUTH.PASSWORD_SAME, HttpStatus.BAD_REQUEST);
    }

    if (dto.newPassword.length < 8) {
      throw new AppError(
        MESSAGES.AUTH.PASSWORD_MIN_LENGTH,
        HttpStatus.BAD_REQUEST,
      );
    }
    ...
    if (!/[A-Z]/.test(dto.newPassword)) {
      throw new AppError(
        MESSAGES.AUTH.PASSWORD_UPPERCASE,
        HttpStatus.BAD_REQUEST,
      );
    }
    ...
    if (!/[0-9]/.test(dto.newPassword)) {
      throw new AppError(MESSAGES.AUTH.PASSWORD_NUMBER, HttpStatus.BAD_REQUEST);
    }
  ...
  async forgotPassword(
    ...
    if (!user) {
      ...
      throw new AppError(
        MESSAGES.AUTH.NO_ACCOUNT_FOUND,
        HttpStatus.UNAUTHORIZED,
      );
    }
