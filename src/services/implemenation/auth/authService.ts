...
import { IAuthService, KycResult, LoginResult, SignupResult, verifyOtpResult } from "../../interface/auth/IAuthService";
...
async googleAuth({ token, role }: { token: string; role: UserRole}): Promise<LoginResult> {
 ...
 logger.info('Google OAuth complete - tokens generated', { 
  ...
  return { email: user.email, otpSent: true };
     ...
     async verifyOtp(dto:verifyOtpDto): Promise<verifyOtpResult> {
      ...
      console.log("verify  otp role",dto.role)
         const repo = this.getRepository(dto.role);
  const storedOtp = await this._redisService.getOtp(dto.email);
  if (!storedOtp) {
    logger.warn('user OTP verification failed - expired', { email: dto.email });
...
async resendOtp(dto: resendOtpDto): Promise<{ success: boolean }> {
  ...
  console.log("resend otp role",dto.role)
   const repo = this.getRepository(dto.role);
 
  let existing = await repo.findByEmail(dto.email);
...
async login(dto: LoginDto): Promise<LoginResult> {  
 ...
 console.log("ZZZZZZZZZZ??",dto.role)
   const repo = this.getRepository(dto.role);
  const user = await repo.findByEmail(dto.email);
  if (!user) {
  ...
  logger.info('user login success', { userId: user._id, email: user.email });
  ...
  user: {
    ...
    ...(user.role === 'LANDLORD' && {
      aadharNumber: (user as ILandlord).kycDetails?.aadhaarNumber || '',
      panNumber: (user as ILandlord).kycDetails?.panNumber || '',
      aadharFrontUrl: (user as ILandlord).kycDocuments?.aadhaarFront || '',
      aadharBackUrl: (user as ILandlord).kycDocuments?.aadhaarBack || '',
      panFrontUrl: (user as ILandlord).kycDocuments?.panCard || '',
    })
  },
  tokens: { accessToken, refreshToken }
};
...
async forgotPassword(dto: forgotPasswordDto): Promise<{ email: string; otpSent: boolean }> {
